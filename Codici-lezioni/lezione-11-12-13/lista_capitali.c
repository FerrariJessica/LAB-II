...
typedef struct capit {
  ...
  double lat;
  double lon;
  struct capit *next; //è una lista semplicemente concatenata quindi è presente solo il campo next e non il prev
} capitale;


//stampa di un oggetto capitale, sul file f, 
void capitale_stampa(capitale *a, FILE *f) {
  fprintf(f,"%20s (%f,%f)\n",a->nome,a->lat,a->lon);
}

//crea un oggetto di tipo capitale
//Dato che le capitali sono nodi della lista dovranno perforza avere il campo next. Alla creazione è inizializzato a NULL
...
//*lis sarebbe il riferimennto alla testa della lista
...
{
  while(lis!=NULL) {
    capitale *tmp = lis->next; // necessario per non perdere il puntatore alla lista
    //devo eliminare nodo per nodo
...
capitale *inserisci_lat(capitale *testa, capitale *c)
{
  ...
  //perciò mi creo un nuovo puntatore per scorrere la lista
  capitale *p = testa;
  while(p->next!=NULL) {
    // controllo se c va inserito tra p e p->next
    assert(c->lat <= p->lat); //controllo che tra i due nodi il p ha la latitudine maggiore in modo di poterlo inserire dopo
    if(c->lat > p->next->lat) {
...
int main(int argc, char *argv[])
{
  ...
  puts("\n------------ test cancellazione --------\n");
  testa = lista_cancella_nome_ric(testa,"Atene");
