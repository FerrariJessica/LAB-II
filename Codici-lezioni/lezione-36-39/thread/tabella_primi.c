...
void *tbody(void *v) {
  ...
  for(int j=d->start;j<d->end;j++)
      if(primo(j)) {
        primi++;
        // faccio la lock sulla tabella, l'aggiorno e rilascio la lock
...
int main(int argc,char *argv[])
{
  ...
  // definizione mutex
  pthread_mutex_t mtxtabella;
  xpthread_mutex_init(&mtxtabella,NULL,QUI);
  ...
  xpthread_mutex_destroy(&mtxtabella,QUI);
  // stampa tabella
