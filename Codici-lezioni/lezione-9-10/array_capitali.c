...
//confronto le latitudini ordinando da nord a sud
int capitale_cmp_lat_ns (capitale *a, capitale *b){
...
//confronto le latitudini ordinando da sud a nord
int capitale_cmp_lat_sn (capitale *a, capitale *b){
  //assert(a != NULL && b!=NULL);
  if(a->lat > b->lat) return 1;
  else if(a->lat < b->lat) return -1;
  return 0;
...
//confronto i nomi
//confronto le latitudini ordinando da nord a sud
int capitale_cmp_name (capitale *a, capitale *b){
  //assert(a != NULL && b!=NULL);
  if(strcmp(a->nome, b->nome) < 0) return -1;
  else if(strcmp(a->nome, b->nome) > 0) return 1;
  return 0;
  //è la solita cosa di scrivere
  //return strcmp(a->nome, b->nome)
}

//esegue il merge di due array di capitali
void merge(capitale *a[], int na, capitale *b[], int nb, capitale *c[], int(*cmp)(capitale*, capitale*)){
  assert (a != NULL && b != NULL && c != NULL);
  int nc = na+nb;
  int ia, ib, ic;
  ia = ib = ic = 0;
  //eseguo il merge riempiendo il vettore c
  for(ic = 0; ic<nc; ic++){
    if(ia == na){
      c[ic] = b[ib++];
    }else if(ib == nb){
      c[ic] = a[ia++];
    }else if (cmp(a[ia], b[ib])<0){
      c[ic] = a[ia++];
    } else {
      c[ic] = b[ib++];
    }
  ...
  //verifico che tutti gli indici siano alla fine
  assert(ia == na);
  assert(ib == nb);
  assert(ic == nc);
}

//ordino un array di puntatori a capitali con il mergesort
void mergesort(capitale *a[], int n, int(*cmp)(capitale *, capitale *)){
  ...
  mergesort(a, n1, cmp);
  mergesort(&a[n1], n2, cmp);

  //ora ho le due metà ordinate e devo unirle con il merge
  capitale **c = malloc(n*sizeof(*c));
  if (c==NULL) termina("Malloc fallita nel mergesort");
  merge(a, n1, &a[n1], n2, c, cmp);
  //copio il risultato da c ad a e libero c
  for(int i=0; i<n; i++){
    a[i]=c[i];
  }
  free(c);
...
int main(int argc, char *argv[])
{
  ...
  mergesort(a, n, &capitale_cmp_lat_ns);
  puts("\n-----array ordinato per latitudine-----\n");
  for(int i=0;i<n;i++){
    capitale_stampa(a[i], stdout);
  }

  mergesort(a, n, &capitale_cmp_name);
