...
// esempio segnali real time
// quando rivece il segnale 14 il thread gestore 
// usa raise per inviare a se stesso un certo numero
// di segnali, che vengono gestiti alle successive
// esecuzione di sigwait. Notare il comprtametne dei 
// segnali real time 
...
void *tgestore(void *v) {
  ...
  while(true) {
    ...
    printf("Thread gestore %d svegliato dal segnale %d\n",gettid(),s);
		if(s==14) {
			raise(SIGINT); raise(SIGUSR1); raise(SIGRTMAX);
			raise(SIGINT); raise(SIGRTMAX);
