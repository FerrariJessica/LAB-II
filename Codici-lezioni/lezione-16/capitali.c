...
// questa fuzione è privata di capitali.c
// nessuno da fuori la può chiamare
// quindi non entra in conflitto con la funzione 
// termina() definito in qsortcap.c  
...
// se errno!=0 stampa anche il messaggio d'errore associato ma
static void termina(const char *messaggio)
{
  if(errno==0) 
     fprintf(stderr,"%s\n",messaggio);
